       ...
       return new Promise((resolve, reject)=>{
        ...
        setTimeout(() => {
            if(jumlahPembayaran > 0 && method.includes(metodePembayaran)){
                resolve(`pembayaran sebesar ${jumlahPembayaran} berhasil menggunakan ${metodePembayaran}`);
            } else{
                reject('pembayaran gagal: jumlah tidak valid atau metode tidak didukung');
            }
        }, 4000);
       })
}
...
async function lakukanPembayaran(jumlahPembayaran, metodePembayaran) {
    try{
        const pembayaran = await prosesPembayaran(jumlahPembayaran, metodePembayaran);
